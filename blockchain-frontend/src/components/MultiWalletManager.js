...
const MultiWalletManager = () => {
  ...
  // WebSocket for general wallet updates
  const { lastMessage: walletMessage } = useWebSocketEnhanced('wallets/', {
    onMessage: (message) => {
      if (message.type === 'wallet_list_update') {
        // Refresh wallet list
        fetchWallets();
      }
    }
  ...
  // WebSocket for selected wallet updates
  const { lastMessage: walletDetailMessage } = useWebSocketEnhanced(
    selectedWallet ? `user_wallet/${selectedWallet.wallet_address}/` : null,
    {
      onMessage: (message) => {
        if (message.type === 'wallet_update') {
          // Update the selected wallet data
          setWallets(prevWallets => {
            return prevWallets.map(wallet => {
              if (wallet.wallet_address === message.wallet_address) {
  ...
  const fetchWallets = async () => {
    try {
      ...
      if (response.data && response.data.length > 0) {
        ...
        // Set the primary wallet as selected or keep current selection
        const primaryWallet = response.data.find(w => w.is_primary);
        if (!selectedWallet && primaryWallet) {
          ...
          // Keep the same selection but update its data
          const updatedWallet = response.data.find(w => w.wallet_address === selectedWallet.wallet_address);
          if (updatedWallet) {
            setSelectedWallet(updatedWallet);
          } else {
            // If the wallet was removed, select the primary
            setSelectedWallet(primaryWallet);
            if (primaryWallet) {
              await fetchTransactionsForWallet(primaryWallet.wallet_address);
            }
  ...
  return (
    <div className="bg-white rounded-lg shadow overflow-hidden">
              ...
              <form onSubmit={handleSendTransaction}>
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    />
                    <p className="mt-1 text-sm text-gray-500">
                      Available balance: {parseFloat(selectedWallet.balance).toFixed(8)}
                    </p>
                  </div>
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
            <div className="p-6">
              ...
              <form onSubmit={handleCreateWallet}>
                <div className="space-y-4">
                  <div>
                    <label htmlFor="wallet_name" className="block text-sm font-medium text-gray-700 mb-1">
                      Wallet Name
                    </label>
                    <input
                      type="text"
                      id="wallet_name"
                      value={createWalletForm.wallet_name}
                      onChange={(e) => setCreateWalletForm({...createWalletForm, wallet_name: e.target.value})}
                      className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                  ...
                  <div>
                    ...
                    <input
                      type="password"
                      id="wallet_passphrase"
                      value={createWalletForm.wallet_passphrase}
                      onChange={(e) => setCreateWalletForm({...createWalletForm, wallet_passphrase: e.target.value})}
                      className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                  ...
                  <div>
                    <label htmlFor="confirm_passphrase" className="block text-sm font-medium text-gray-700 mb-1">
                      Confirm Passphrase
                    </label>
                    <input
                      type="password"
                      id="confirm_passphrase"
                      value={createWalletForm.confirm_passphrase}
                      onChange={(e) => setCreateWalletForm({...createWalletForm, confirm_passphrase: e.target.value})}
                      className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                ...
                <div className="mt-6 flex justify-end space-x-3">
                  <button
                    type="button"
                    onClick={() => setCreateWalletDialogOpen(false)}
                    className="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                  >
                    Cancel
                  </button>
      ...
      {backupDialogOpen && selectedWallet && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
            <div className="p-6">
              <h3 className="text-lg font-medium text-gray-900 mb-4">Backup Wallet: {selectedWallet.wallet_name}</h3>
              
              <div className="mb-4">
                <p className="text-sm text-gray-600">
                  Creating a wallet backup is crucial for securing your funds. Enter your wallet passphrase
                  to create an encrypted backup that can be used to recover your wallet if needed.
                </p>
              </div>
              
              <form onSubmit={handleBackupWallet}>
                <div>
                  <label htmlFor="backup_passphrase" className="block text-sm font-medium text-gray-700 mb-1">
                    Wallet Passphrase
                  </label>
                  <input
                    type="password"
                    id="backup_passphrase"
                    value={backupForm.wallet_passphrase}
                    onChange={(e) => setBackupForm({...backupForm, wallet_passphrase: e.target.value})}
                    className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                    placeholder="Enter your wallet passphrase"
                    required
                  />
